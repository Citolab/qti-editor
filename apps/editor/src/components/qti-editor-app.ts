...
export class QtiEditorApp extends LitElement {
  ...
  private onMetadataChange(event: Event) {
    ...
    const currentTitle = (view.state.doc.attrs.title as string) ?? '';
    const currentIdentifier = (view.state.doc.attrs.identifier as string) ?? '';

    let nextIdentifier = detail.identifier;
    if (this._docIdentifierAutogenerated) {
      if (detail.identifier !== currentIdentifier) {
        this._docIdentifierAutogenerated = false;
      } else {
        const slug = slugifyTitle(detail.title);
        nextIdentifier = slug ? `${slug}-${this._docUuid}` : '';
      }
    }

    if (detail.title === currentTitle && nextIdentifier === currentIdentifier) return;

    const tr = view.state.tr;
    if (detail.title !== currentTitle) tr.setDocAttribute('title', detail.title);
    if (nextIdentifier !== currentIdentifier) tr.setDocAttribute('identifier', nextIdentifier);
    view.dispatch(tr);
  }
  ...
  override render() {
    const docItem = this.itemContext.items?.[0];
    const showMetadataValidation = docItem != null && this.hasMetadataSelection();

...
const qtiNodes = Object.fromEntries(
  [...selectPointInteractionDescriptor.nodeSpecs, ...qtiRubricBlockDescriptor.nodeSpecs].map(({ name, spec }) => [
    name,
    spec
  ])
);
...
/** The editor schema used for the ITEM016 roundtrip. */
...
const EDITABLE_ATTRS = Object.fromEntries(
  Object.values(selectPointInteractionDescriptor.attributePanelMetadata ?? {}).map(metadata => [
...
export const importItem016 = (): ProseMirrorNode =>
...
/** Mount the ITEM016 editor into `container`, optionally wiring the attributes panel. */
export const mountEditor = (container: HTMLElement, options: { panelEl?: HTMLElement } = {}): EditorView => {
  ...
  const view = new EditorView(container, {
    state: EditorState.create({ doc: importItem016(), schema, plugins }),

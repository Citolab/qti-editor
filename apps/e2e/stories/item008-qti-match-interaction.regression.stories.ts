/**
 * Pure-ProseMirror QTI roundtrip regression for ITEM008 (match — directedPair multiple).
 *
 *   ITEM008.xml (raw import)
...
const editor = createRegressionEditor({
  ...
  sourceXML,
  transforms: schema => [roundtripMatch, roundtripItemBody, ensureInteractionPrompts(schema)],
  extraNodes: { qtiMediaStub }
});

export const { schema, exportAssessmentItemDoc, mountEditor } = editor;

/** Import ITEM008.xml into a ProseMirror document (raw QTI → roundtrip-xml → PM doc). */
export const importItem008 = editor.importItem;
...
const meta: Meta = {
  ...
  excludeStories: ['schema', 'importItem008', 'exportAssessmentItemDoc', 'mountEditor']
};
export default meta;

export const RoundtripItem008: StoryObj = {
  render: () => {

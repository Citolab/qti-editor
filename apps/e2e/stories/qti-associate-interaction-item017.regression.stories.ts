/**
 * Pure-ProseMirror QTI roundtrip regression for ITEM017 (match — directedPair multiple).
 *
 *   ITEM017.xml (raw import)
...
const qtiNodes = Object.fromEntries(
  [...associateInteractionDescriptor.nodeSpecs, ...qtiRubricBlockDescriptor.nodeSpecs].map(({ name, spec }) => [
    name,
    spec
  ])
);
...
export const importItem017 = (): ProseMirrorNode =>
...
const meta: Meta = {
  ...
  excludeStories: ['schema', 'importItem017', 'exportAssessmentItemDoc', 'mountEditor']
};
export default meta;

export const RoundtripItem017: StoryObj = {
  render: () => {

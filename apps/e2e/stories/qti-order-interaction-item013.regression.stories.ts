...
import sourceXML from '@qti-editor/example-items/ITEM013.xml?raw';
...
const qtiNodes = Object.fromEntries(
  [...orderInteractionDescriptor.nodeSpecs, ...qtiRubricBlockDescriptor.nodeSpecs].map(({ name, spec }) => [
    name,
    spec
  ])
);
...
export const importItem013 = (): ProseMirrorNode =>
  importItemFromString(sourceXML, schema, {
    assetBasePath: '/qti/kennisnet',
    transforms: [roundtripOrder, roundtripItemBody, ensureInteractionPrompts(schema)]
  });
...
export const mountEditor = (container: HTMLElement, options: { panelEl?: HTMLElement } = {}): EditorView => {
  ...
  const view = new EditorView(container, {
    state: EditorState.create({ doc: importItem013(), schema, plugins }),

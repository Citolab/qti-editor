...
This guide mirrors the [`qti-example-editor`](https://github.com/Citolab/qti-editor/tree/main/apps/qti-example-editor) boilerplate — a minimal pure-ProseMirror QTI roundtrip editor. Use it as the reference shape for your own integration.
...
export const qtiPlugins: Plugin[] = [
  ...
  // Resets the selection back to plain paragraphs, skipping over any interaction it spans. An
  // authoring affordance like the decorators below — drop this line for a read-only or player host.
  keymap({ 'Mod-\\': clearFormatting() }),
  ...descriptors.flatMap(d => d.pluginFactories?.map(factory => factory()) ?? []),
  // Opt-in authoring affordances (per-choice remove button, add button, settings pill shown on
  // click) — currently shipped by the choice descriptor. Drop this line for a read-only or player
  // host.
...
| `decoratorPluginFactories` | Optional, opt-in authoring affordances (per-choice remove button, add button, settings pill shown on click) — see [Editor decorations](#editor-decorations-opt-in) below |
...
## Editor decorations (opt-in)

`qtiPlugins` above already folds in `listInteractionDecoratorPluginFactories()`, so the boilerplate's
choice interaction gets its remove button, add button and settings pill for free — the add button
and pill appear once you click into the interaction, and stay until you type, press `Escape`, or
click elsewhere. Pair it with the matching stylesheet:

```css
@import '@citolab/prose-qti/decorations.css';
```

The settings pill dispatches a bubbling, composed `QTI_OPEN_NODE_SETTINGS_EVENT` rather than opening
...
See [Choice Interaction](/docs/qti-interactions/choice/#built-in-editor-decorations-opt-in) for what the
...
const clearFormattingItem = new MenuItem({
  run:    clearFormatting(),
  enable: state => clearFormatting()(state),
  label:  'Clear formatting',
  title:  'Clear formatting',
});

// Add `menuBar({ content: [[insertInteractionDropdown], [clearFormattingItem], …] })` to your plugin stack.
```

## Attributes panel

The boilerplate ships its own local attributes-panel ProseMirror plugin — a small file that renders the selected node's attribute chain into a side panel and applies edits via transactions. See [`attributes-panel-plugin.ts`](https://github.com/Citolab/qti-editor/blob/main/apps/qti-example-editor/src/components/attributes-panel-plugin.ts) in the boilerplate; it consumes the `editableAttrs` allowlist exported from `prosemirror-qti.ts`.

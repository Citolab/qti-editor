...
Base prose nodes and marks (`paragraph`, `text`, `image`, marks) come from `qtiBasicNodes`/`qtiBasicMarks` in `@citolab/prose-qti` — see [QTI Base Schema](/docs/packages/qti-base-schema/) — rather than importing `prosemirror-schema-basic` directly, so image `width`/`height` survive QTI XML import and roundtrip:

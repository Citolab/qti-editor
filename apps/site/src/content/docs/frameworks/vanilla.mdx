...
export const qtiPlugins: Plugin[] = [
  ...
  // Opt-in authoring affordances (a per-choice remove button, a click-activated add button,
  // settings pill) — currently shipped by the choice descriptor. Drop this line for a read-only
  // or player host.
...
| `decoratorPluginFactories` | Optional, opt-in authoring affordances (a per-choice remove button, a click-activated add button, settings pill) — see [Editor decorations](#editor-decorations-opt-in) below |
...
choice interaction gets its per-choice remove button, click-activated add button and settings pill
for free. Pair it with the matching stylesheet:

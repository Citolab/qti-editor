...
export const qtiPlugins: Plugin[] = [
  ...
  // Opt-in authoring affordances (hover boundary, add/remove buttons, settings pill) — currently
  // shipped by the choice descriptor. Drop this line for a read-only or player host.
...
| `decoratorPluginFactories` | Optional, opt-in authoring affordances (hover boundary, add/remove buttons, settings pill) — see [Editor decorations](#editor-decorations-opt-in) below |

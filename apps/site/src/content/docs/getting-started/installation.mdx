...
Import `@citolab/prose-qti/core-css.css` wherever your app renders the editor's document — it is a mandatory plain-CSS stylesheet (no Tailwind or PostCSS plugins required) that styles interaction element backgrounds, spacing, the rubric-block boundary, and a ProseKit placeholder fix:

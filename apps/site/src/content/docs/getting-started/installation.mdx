...
QTI Editor is split into focused packages so you only install what you need. The [`qti-example-editor`](https://github.com/Citolab/qti-editor/tree/main/apps/qti-example-editor) boilerplate is the reference shape; this page lists the packages it uses and how to install them.
...
Interactions that ship an editor decorator (currently choice — see [Choice
Interaction](/docs/qti-interactions/choice/#built-in-editor-decorations-opt-in)) need a second, opt-in
stylesheet on top of the mandatory one above:
...
| `@citolab/prose-qti/decorations.css` | Styles for the opt-in editor decorators (per-choice remove button, add button, settings pill shown on click) | When you also install `decoratorPluginFactories` for an interaction. |

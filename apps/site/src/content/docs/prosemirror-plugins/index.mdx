...
`@citolab/prose-extensions` provides generic editor behavior that sits alongside `@citolab/prose-qti`. Use it when you need block selection, node attribute synchronization, semantic paste cleanup, schema-gap detection, or a clear-formatting reset that does not depend on QTI semantics.
...
- [Schema Gaps](/docs/prosemirror-plugins/schema-gaps/) - finds and preserves markup a schema cannot represent
- [Clear Formatting](/docs/prosemirror-plugins/clear-formatting/) - strips marks and flattens block structure back to plain paragraphs

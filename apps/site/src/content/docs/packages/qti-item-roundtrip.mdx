...
There used to be an `ensureInteractionPrompts` transform that synthesised an empty `<qti-prompt><p/></qti-prompt>` on import because some editor schemas required one on `order`, `match`, `gap-match`, and other block interactions. Making `qtiPrompt` optional in those schemas removed the need for it entirely; only its export-side counterpart remains:

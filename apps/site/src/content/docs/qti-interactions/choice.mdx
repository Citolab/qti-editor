...
Every interaction gets a decorator — the click-to-activate wash and the settings pill are shared by all of them. Choice is the one with its own add/remove affordances (the per-choice `×` and the trailing `+`) on top of that shared base, since "another one of these" is specific to each interaction and the rest have nothing type-specific yet.

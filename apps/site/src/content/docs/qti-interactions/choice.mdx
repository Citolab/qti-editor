...
Rather than building the affordance above yourself, you can opt into the ready-made one: hovering a choice reveals its remove (`×`) button, and clicking into a choice interaction paints a light gray wash and shows a trailing `+` that appends a new choice plus a settings pill above the interaction. Typing or pressing `Escape` hides the `+` and the pill again; moving the caret with the arrow keys does not — only the next click does. These are view-only decorations — they never enter the document, so exported XML is unaffected — and they are **not** included by default, since a read-only or player host must not get authoring chrome it never asked for.

Every interaction gets this pill (select, settings, copy, delete); choice is the one with the extra per-choice `×`/`+` on top, since "another one of these" means something different for each interaction type.
...
Every interaction registers a decorator through the same `decoratorPluginFactories` plumbing on `InteractionDescriptor`; choice is the only one that adds anything beyond the pill.

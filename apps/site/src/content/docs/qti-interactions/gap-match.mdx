...
- Add a gap text by pressing `Enter` at the end of one — a new sibling gap text is inserted automatically. A single gap text is a valid pool; you don't need a second one to start.
- Backspace in an empty gap text removes it, as long as it isn't the only one left in the pool.

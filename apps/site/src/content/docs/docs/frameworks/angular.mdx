...
- we recommend using [ProseKit](https://prosekit.dev) for ready-made UI components. If desired, install `prosekit` directly from the vendor
- when using ProseKit, keep a small local ProseKit helper layer in your Angular app for editor assembly and event wiring

We've provided a ready-made [Angular integration example](https://github.com/Citolab/qti-editor-angular) to serve as inspiration on how to wire things up.

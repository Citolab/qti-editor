...
export const NOTES: Record<string, string> = {
  ...
  image: "Inline, and modelled on `prosemirror-schema-basic` rather than ProseKit — see prosekit-divergences.md. ProseKit's image is a BLOCK with `src`/`width`/`height` and no `alt`, and all three diverge from what QTI needs: the XSD does not allow `img` as a child of `qti-item-body` (it is phrasing content and wants a block parent, so a block image can only serialise to something the schema rejects), a text alternative is required, and an image in running text is ordinary assessment content. `width`/`height` are strings, not ProseKit's numbers — the sample items use `width=\"100%\"` as well as `width=\"250\"`, and a numeric attr drops the percentage. Parsing reads the attributes; ProseKit's derives size from `getBoundingClientRect()`, which is 0 on the detached DOM an import runs against, so the authored size was lost. (imgSelectPoint stays a separate leaf node — see its own note.)",
  ...
  qtiGapMatchInteraction: 'The draggable sources come first as a flat list, then the prose holding the gaps. `qti-gap-text` is `+`, as the XSD has it. The editor required two for a while — one source makes a degenerate interaction — but that is a judgement about a finished item, and the editor is where items are half-finished: making the first gap out of prose leaves exactly one source, and undoing a gap has to be able to get back to one.',

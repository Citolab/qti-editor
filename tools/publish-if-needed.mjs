...
if (publish.status !== 0) {
  ...
  if (isRegistryPermission404) {
    ...
    console.error(
      'This usually means the current npm account or token does not have publish rights for the @citolab scope.'
    );
    console.error('Verify with `npm whoami` and confirm that account can publish @citolab packages.');
  }
}

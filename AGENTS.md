...
- Keep domain logic in package modules. Integration and composition decisions belong to the consuming application, which now lives in its own repository — resist pulling app-shaped concerns back into these packages.
...
- **Applies to**: Lit components anywhere in this repo — `packages/prose-qti/src/components/*/` and
  any `apps/*/src/components/*/`. The extracted editor app follows the same convention.

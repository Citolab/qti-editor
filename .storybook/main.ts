...
const config: StorybookConfig = {
  ...
  async viteFinal(config: any) {
    ...
    return {
      ...
      publicDir: false,
      /*
       * No tailwindcss() here on purpose. Tailwind and daisyUI belong to apps/site, which imports
       * "tailwindcss" from its own style.css and runs the plugin in its own Vite config. Nothing
       * Storybook loads does: preview.ts pulls modern-normalize, the qti-components theme and
       * prose-qti's core-css, and the regression stories add only their own CSS.
       *

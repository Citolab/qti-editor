...
## 26. The generic interaction decorator overwrote `qti-inline-choice-interaction`'s own `anchor-name` — **fixed**

`qti-inline-choice-interaction`'s dropdown popover (finding 25's `MenuAutoSizeMixin` migration) anchors itself to its trigger element via CSS anchor positioning, which needs `anchor-name` set on that same element. `createInteractionDecoratorPlugin` (`components/shared/extensions/interaction-decorations.ts`) also writes `anchor-name` — for the floating node-action pill — as an inline style via `Decoration.node`, which lands on the interaction's own custom element when it has no NodeView. An inline style always wins over a `:host` rule, so the decorator's `anchor-name` silently overwrote the popover's, and the dropdown's `anchor()` references resolved to nothing — it opened pinned to the viewport's top-left corner instead of below its trigger.
...
## 28. New "clear formatting" command silently no-op'd on an ordinary selection — **fixed**

`clearFormattingInRange`'s (`packages/prose-extensions/src/prosemirror/clear-formatting/clear-formatting.ts`) first version only dissolved a wrapper or reset a textblock when the passed range reached the node's own outer position (`pos`/`pos + nodeSize`). A wrapper's immediate child is usually itself a non-leaf node — `blockquote`'s child is a paragraph, a list-item's child is a paragraph too — so `pos + 1` only reaches that child's opening delimiter, not its text. A selection produced by clicking into a blockquote and selecting its line, or triple-clicking a heading, never reaches past that structural delimiter, so it never satisfied the containment check: "select this quote's text and clear formatting" silently did nothing. Every test written against the function up to that point happened to select the whole document (`apply(doc, 0, doc.content.size)`) with the wrapper as the document's first child, which trivially satisfied the check and masked the bug.

- **Resolution**: containment is now checked against the innermost position a text selection can actually reach — `firstReachablePos`/`lastReachablePos` step inward from `pos`/`pos + nodeSize` through every non-leaf, non-text child. A regression case pins a heading preceded by a sibling (so its own start position is `> 0`) selected only by its own text, which the old whole-document-selection tests could not have caught.
- **Where to look**: `packages/prose-extensions/src/prosemirror/clear-formatting/clear-formatting.ts` (`firstReachablePos`, `lastReachablePos`); `clear-formatting.browser.test.ts` for the regression cases.

...
`src/components/shared/mixins/interaction-panel.ts`'s `InteractionPanel` is the shared base class for a dropdown/panel-style interaction: it tracks open/closed state and drives it from pointerdown, focusin, selectionchange and Escape, with each trigger gated behind an overridable `should...` hook so a subclass can opt out of one without reimplementing the rest. `inline-choice` is currently its only consumer. The base's default `shouldClosePanelOnOutsidePointerDown()` returns false while the host sits inside a ProseMirror surface (pinning the panel open there — a runtime host outside the editor still closes on outside click), but `qti-inline-choice-interaction` overrides it to always close on outside click, and on close blurs both itself and its shadow root's `delegatesFocus` active element in `onPanelOpenChanged` — without that, the trigger element keeps its focused styling after the click that closed the dropdown moves focus elsewhere. It still reflects open/closed through `ElementInternals.states` as `:state(open)`; `core-css/interaction-state-layout.css` uses that to force `overflow: visible` on a containing ProseMirror table while the dropdown is open, a table-specific fix that predates the dropdown moving to a native popover (below) and is superseded there for inline-choice's own clipping, since a top-layer popover cannot be clipped by an ancestor's `overflow` in the first place.

`qti-inline-choice-interaction` used to lay its dropdown out as an in-flow grid item — deliberately out of upstream's popover model — so the widest option could size the closed trigger without measuring. That traded away robustness against clipping: a ProseMirror table, the editor's own card, or any other scrolling ancestor could still cut the menu off, not only the table case the `:state(open)` rule above was built for. The component now mixes in `@qti-components/interactions-core`'s `MenuAutoSizeMixin` and renders `[part="menu"]` as a native `popover="manual"`, toggled via `showPopover()`/`hidePopover()` in `#syncPopover()` (called from `updated()` on `_panelOpen` changes) — the same layout model upstream uses, so the menu always renders in the top layer above any clipping ancestor. Trigger width is restored by `MenuAutoSizeMixin` measuring option rows and writing the result inside the element's own shadow root (never onto the ProseMirror-managed host), which is what keeps the measurement from tripping the freeze the old in-flow layout was built to avoid — see `#estimateOptimalWidth`'s history in `qti-inline-choice-interaction.ts` for that incident. Because the mixin measures a row by writing `style.width` on it and reading it back, and the real options are ProseMirror-managed light DOM where a style attribute the schema doesn't declare gets reconciled away (re-firing `slotchange`, re-triggering measurement, looping forever), `menuAutoSizeRows()` and `withMenuMeasurable()` are overridden to clone light-DOM rows into a throwaway `[part="measure-sandbox"]` in the shadow root instead of measuring them in place, discarding the clones once the mixin is done reading them.
...
`src/core-css/core-css.css` — the mandatory stylesheet for rendering the editor's PM document, published as the `./core-css.css` subpath (`@citolab/prose-qti/core-css.css`). It is a compatibility entry point that `@import`s three split source files kept separate for review: `core-layout.css` (mandatory structural fixes — interaction backgrounds/spacing, the rubric-block boundary, the ProseKit placeholder fix), `editor-states.css` (editor-only `:state(...)` hooks for gaps, choices and the order interaction — `pending`, `selected` and `linked` are the states this package owns, since they are affordances the runtime has no reason to know about; `drag`, `checked` and `disabled` stay qti-theme's. `selected` (a picked-up drag source) and `pending` (a drop target, or the order interaction host, while a source is pending) both paint an outline plus a `--qti-primary`-tinted background — solid for `selected`, dashed for `pending` — so the two states of a gap-match/match/associate drag read as a single accent-color affordance; `linked` is still documented but deliberately left unpainted until a rule for it is reviewed), and `interaction-state-layout.css` (layout consequences of an interaction's own `:state(...)` — see the `InteractionPanel` paragraph below for its one current rule). Every app that renders the editor must import `core-css.css` alongside `@qti-components/theme`; nothing outside this package imports the three source files directly.
...
The `+` widget is anchored below the last choice and, like every decoration, out of document flow, so nothing reserved space for it — in a document with content right after a `qti-choice-interaction`, the `+` painted on top of that content instead of clearing it. `core-css/decorations.css` gives `qti-choice-interaction.qti-interaction--active` extra `margin-bottom`, sized to clear the `+`'s own offset plus height, and transitions it so the layout doesn't shift abruptly the moment a choice is clicked into. Only `qti-choice-interaction` needs this: it is the only interaction whose generic wash-and-pill decorator also emits a trailing widget below the interaction itself.

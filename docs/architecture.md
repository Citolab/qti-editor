...
- `schemaToJson(schema?)` (`@citolab/prose-qti/schema` and `/node`) projects a schema's grammar to a plain JSON object on demand — nodes, marks, content expressions, group membership, attribute defaults/requiredness. Replaces the generator removed in 6cdc6d5 that wrote `schema/content-model.json` to disk with 11 committed fixtures; reading the live schema on demand means there is nothing left to drift, so no writer, fixture or fingerprint comes back. Counterpart to the repo's `custom-elements.json` (generated by `pnpm run cem`) — that manifest says which `qti-*` elements exist and what attributes they take, this says what may nest inside what, and neither subsumes the other.
...
An element or attribute missing from `custom-elements.json` after running `pnpm run cem` means the JSDoc is missing, not that the filter dropped it — the script logs excluded editor-only tags (e.g. `dummy-drag`, which has no QTI counterpart) and any attribute-less element as a warning.

...
export class LitToolbar extends LitElement {
  ...
  override render() {
    ...
    const configuredMenus: ToolbarInsertMenu[] =
      this.insertMenus ??
      (this.getInsertItems
        ? [
            {
    ...
    return html`
      <div
        class="z-2 sticky top-0 bg-white dark:bg-gray-950 box-border border-gray-200 dark:border-gray-800 border-solid border-l-0 border-r-0 border-t-0 border-b flex flex-wrap gap-1 p-2 items-center"
      >
          ...
          return html`
            ...
            >
              ...
              <prosekit-popover-content
                class="flex flex-col gap-1 p-2 text-sm min-w-48 z-10 box-border rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 shadow-lg [&:not([data-state])]:hidden will-change-transform motion-safe:data-[state=open]:animate-in motion-safe:data-[state=closed]:animate-out motion-safe:data-[state=open]:fade-in-0 motion-safe:data-[state=closed]:fade-out-0 motion-safe:data-[state=open]:zoom-in-95 motion-safe:data-[state=closed]:zoom-out-95 motion-safe:data-[state=open]:animate-duration-150 motion-safe:data-[state=closed]:animate-duration-200"
              >
                ${menuItems.map(
                  item => html`
                    <button

...
const NODE_RANGE_SELECTION_JSON_ID = 'node-range';
const NODE_RANGE_SELECTION_REGISTRATIONS_KEY = Symbol.for('qti-editor.node-range-selection.registrations');
...
export class NodeRangeSelection extends Selection {
  ...
  override toJSON(): { type: string; anchor: number; head: number } {
    return { type: NODE_RANGE_SELECTION_JSON_ID, anchor: this.$anchorNode.pos, head: this.$headNode.pos };
  }
...
registerNodeRangeSelection();

function isNodeRangeSelection(selection: Selection): selection is NodeRangeSelection {
  if (selection instanceof NodeRangeSelection) return true;

  const maybeSelection = selection as Partial<NodeRangeSelection>;
  const selectionJson =
    typeof maybeSelection.toJSON === 'function'
      ? (maybeSelection.toJSON() as { type?: unknown } | undefined)
      : undefined;

  return (
    selectionJson?.type === NODE_RANGE_SELECTION_JSON_ID &&
    typeof maybeSelection.$anchorNode?.pos === 'number' &&
    typeof maybeSelection.$headNode?.pos === 'number'
  );
}

function registerNodeRangeSelection(): void {
  const globalScope = globalThis as Record<symbol, WeakSet<(typeof Selection)['jsonID']> | undefined>;
  const registrations =
    globalScope[NODE_RANGE_SELECTION_REGISTRATIONS_KEY] ??
    (globalScope[NODE_RANGE_SELECTION_REGISTRATIONS_KEY] = new WeakSet());

  if (registrations.has(Selection.jsonID)) {
    (NodeRangeSelection.prototype as { jsonID?: string }).jsonID = NODE_RANGE_SELECTION_JSON_ID;
    return;
  }

  try {
    Selection.jsonID(NODE_RANGE_SELECTION_JSON_ID, NodeRangeSelection);
  } catch (error) {
    if (
      !(error instanceof RangeError) ||
      !String(error.message).includes(`Duplicate use of selection JSON ID ${NODE_RANGE_SELECTION_JSON_ID}`)
    ) {
      throw error;
    }
    (NodeRangeSelection.prototype as { jsonID?: string }).jsonID = NODE_RANGE_SELECTION_JSON_ID;
  ...
  registrations.add(Selection.jsonID);
}
...
export const blockSelectPlugin = new Plugin({
  ...
  props: {
    handleKeyDown(view, event) {
      ...
      if (event.key === 'Escape' || (event.key === 'Enter' && isNodeRangeSelection(selection))) {
        if (isNodeRangeSelection(selection) || !selection.empty) {
    ...
    handleDOMEvents: {
      mousedown(view, event) {
        ...
        if (event.shiftKey && isNodeRangeSelection(view.state.selection)) {
            ...
            view.dispatch(
              view.state.tr.setSelection(
                NodeRangeSelection.create(
                  view.state.doc,
                  view.state.selection.$anchorNode.pos,
                  block.pos
                )
              )
            );

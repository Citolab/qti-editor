...
type SelectPointAttrsChangeDetail = {
  ...
  tagName: 'img-select-point';
  attrs: Record<string, unknown>;
};

export class ImgSelectPoint extends LitElement {
  ...
  @state()
  protected imageSourceError: string | null = null;
  ...
  async #applyImageUrl(event: Event) {
    event.preventDefault();

    const value = this.#urlInputRef.value?.value?.trim();
    if (!value) {
      this.imageSourceError = 'Enter an image URL or data URL.';
      return;
    }
    ...
    try {
      ...
      this.imageSourceError = 'Could not load image from that URL.';
    }
  }
  ...
  override render() {
    ...
    return html`
      <div class="toolbar">
        <button type="button" @click=${this.#openImageDialog}>Add image</button>
      ...
      <dialog ${ref(this.#dialogRef)}>
        <p class="dialog-title">Add Image Source</p>
        ${this.imageSourceError ? html`<p class="dialog-error">${this.imageSourceError}</p>` : nothing}
        <div class="dialog-row">
          <button type="button" @click=${this.#openFilePicker}>Upload file</button>
          <span style="font-size:12px;color:#4b5563;">or paste a URL/data URI</span>
        </div>
        <form @submit=${this.#applyImageUrl}>
          <div class="dialog-row">
            <input
        ...
        <div class="dialog-row" style="justify-content:flex-end;margin-bottom:0;">
          <button type="button" @click=${this.#closeImageDialog}>Close</button>
        </div>
      </dialog>
...
customElements.define('img-select-point', ImgSelectPoint);

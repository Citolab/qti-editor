...
export const RoundtripItem004: Story = {
  render: () => {
    ...
    const init = async (container: HTMLElement) => {
      const loaded = (await qtiTransformItem().load('/qti/kennisnet/ITEM004.xml', { shuffle: false })).path('/qti/kennisnet');
    ...
    const logExport = () => {
      ...
      const xml = qtiItemFromProsemirror(
        ...
        schema,
        [textEntryInteractionDescriptor.composerMetadata],
      );
    ...
    return html`
      <div style="max-width: 850px; margin: 40px auto; padding: 0 20px; font-family: system-ui;">
        <h3>QTI Roundtrip: load → import → edit → export (ITEM004)</h3>
        ...
        <p style="color: #666; font-size: 0.9rem;">
          Loads <code>/qti/kennisnet/ITEM004.xml</code>, runs

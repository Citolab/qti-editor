...
```ts
import { qti3ToPm, pmToQti3, htmlToPm, pmToHtml, validateHtml, createQtiSchema, schemaToJson } from '@citolab/prose-qti/node';
```

The Node-only entry point. It installs a `linkedom` DOM shim as an import side
effect, which is why the QTI ↔ ProseMirror conversion functions live on their
own subpath rather than the package root — a browser consumer importing
`@citolab/prose-qti` never pulls the DOM shim into a bundle. Also re-exports
`createQtiSchema` (the roundtrip schema every conversion function here uses)
and:

#### schemaToJson(schema?: Schema): SchemaJson

Serializes a ProseMirror schema's grammar — nodes, marks, content
expressions, group membership, attribute defaults/requiredness — to a plain
JSON-able object. Defaults to `createQtiSchema()` if no schema is passed.
...
The counterpart to the repo's `custom-elements.json` (generated by `pnpm run
cem` from the interaction components' JSDoc): the manifest says which `qti-*`
elements exist and what attributes they take, this says what may nest inside
what (e.g. `qtiPrompt? qtiSimpleChoice+`). Neither subsumes the other, since a
custom elements manifest has no way to express a content model. `parseDOM`
and `toDOM` are excluded from the output — they're functions, and describe
serialization rather than the document model — except that each node's
`tagName` is read back out of `toDOM` for convenience.

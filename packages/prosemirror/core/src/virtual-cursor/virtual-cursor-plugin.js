 ...
 * ```
 */
export function createVirtualCursor(options) {
...
function checkInclusive(schema, skipWarning) {
    for (const [mark, type] of Object.entries(schema.marks)) {
        if (type.spec.inclusive === false && !skipWarning.includes(mark)) {
            console.warn(`[prosemirror-virtual-cursor] Virtual cursor does not work well with marks that have inclusive set to false. Please consider removing the inclusive option from the "${mark}" mark or adding it to the "skipWarning" option.`);
        }
    }
}
